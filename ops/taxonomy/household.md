...
- [x] **1. צלחת יחיד** - תוקן: `צלחת` (יחיד) נוסף לרול ניקיון, עם שומר חומר (זכוכית/פורצלן/קרמיקה/חרסינה/מלמין) כדי לא לגנוב סט אמיתי; `צלחות` (רבים) נוסף לרול בית וכלים עבור סטים שכן נושאים חומר.
- [x] **2. `disposable-plates` תופס סט אמיתי דרך "סט"** - לא ניתן לתקן ב-general.json (מחוץ לתחום), אבל נוסף עוקף ברמת categorize.js: `conceptRejected()` דוחה את המושג `disposable-plates` לפי id כששם המוצר מזכיר חומר (אותו pattern כמו MUSHROOM_CONCEPT_IDS הקיים) - נופל דרך ל-CATEGORY_RULES שמנתב נכון לבית וכלים.
- [x] **3. ג'ל אנרגיה** - תוקן: חריג ברול ניקיון + מילת מפתח חדשה ברול פארם ותוספים (`(?:ג['\`]?ל|גל)\s?אנרגיה`, כולל הגרש האחורי `` ` ``).
- [x] **4. גל גבות** - תוקן: `ג'?ל גבות` נוסף לרול טיפוח ויופי (נבדק לפני ניקיון).
- [x] **5. ג'ל ניקוי פנים / לעור** - תוקן חלקית: `ניקוי פנים` ו-`ניקוי לעור` נוספו לרול טיפוח ויופי - מכסה את רוב המקרים. "גל ניקוי בוקר" (בלי "פנים"/"לעור" בשם) עדיין לא מכוסה - מקרה בודד, לא הוצדק כלל כללי רחב יותר.
- [x] **6. סבון פנים** - תוקן: `סבון פנים` נוסף לרול טיפוח ויופי, עם אותו שומר שמפו/תחליב רחצה כדי לא לגנוב מארזי "שמפו+ג'ל רחצה+סבון פנים" 3-ב-1.
- [x] **7. כפפות אלוורה** - תוקן: `כפפות אלוורה` נוסף לרול טיפוח ויופי.
