...
export function createApp({ dataDir = path.join(ROOT, 'data'), stateFile = path.join(dataDir, 'runtime', 'state.json'), persist = !process.env.VERCEL, logger = console } = {}) {
  ...
  router.get('/api/chains', () => ({
    chains: chainsForCompare.map((c) => ({ id: c.id, name: c.name, color: c.color, website: c.website, verified: c.verified, inStoreOnly: !!c.inStoreOnly, pickupOnly: !!c.pickupOnly, note: c.note ?? null, branches: c.branches })),
  }));
