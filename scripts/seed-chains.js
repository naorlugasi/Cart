#!/usr/bin/env node
/**
 * Generates deterministic per-chain catalogs (data/catalogs/<chain>.json) from the unified
 * product catalog. In production these files come from the price-transparency XML files
 * (see scripts/import-prices.js); this generator only exists so the MVP runs offline.
 */
...
for (const [chainId, profile] of Object.entries(PROFILES)) {
  ...
  const catalog = { chainId, generatedAt: '2026-09-07T00:00:00.000Z', source: 'scripts/seed-chains.js', items };
  writeFileSync(path.join(dataDir, 'catalogs', `${chainId}.json`), JSON.stringify(catalog, null, 2) + '\n');
  console.log(`${chainId}: ${items.length} items`);
}
