...
(המחירון של האונליין שונה לפעמים מזה של הסניפים). **המחירון והקטלוג נבנים אך ורק מהקבצים המפורסמים, ובריצה היומית אין שום פנייה לאתרי הרשתות.** (כלי הביקורת הידניים `audit:online-check` ו-`audit:shufersal-codes` משווים קובץ לאתר לפי בקשה בלבד):
...
| ויקטורי, מחסני השוק | laibcatalog.co.il | 097 "אינטרנט" (סניף האונליין של כל אחת; ה-API של Laib עונה מה-runner, 17.9.2026) |
