...
|---|---|---|---|---|---|
| שופרסל | SAP Hybris | כן (JSESSIONID; דף העגלה מפנה ל-login, הפריטים נשמרים בסשן ומתמזגים אחרי התחברות) | `POST /online/he/cart/add?cartContext[openFrom]=CATALOG&cartContext[recommendationType]=REGULAR`, JSON `{productCodePost, productCode, sellingMethod:"BY_UNIT", qty, frontQuantity, comment, affiliateCode}`, כותרת `CSRFToken` מ-`<meta name="_csrf">`; התשובה היא HTML של המיני-עגלה | קוד המוצר באתר `P_<ItemCode>`; ברקודים תחת הקידומת 729000 מקוצרים למספר שאחרי הקידומת (`7290000066318` → `P_66318`, `7290004131074` → `P_4131074`), שאר הברקודים כמו שהם (`P_7290107932080`). הכלל מיושם ב-`scripts/fetch-prices.mjs` ואומת מול ה-API של העגלה | ✅ תוסף, `e2e-shufersal.json` |
...
| `stale_injector` | שער הגרסה סירב לסימנייה (ראו למטה) | לא נשלחה אף בקשת הוספה; הלקוח מתבקש לגרור סימנייה חדשה |
...
## גרסאות הסימנייה ושער הגרסה (22.9) - חוזה

הסימנייה היא **עותק קפוא** של `src/handoff/injector.cjs` שיושב בשורת הסימניות של הלקוח. היא לא מתעדכנת
לבד: לקוח שגרר אותה לפני חודש ירוץ עם הקוד של לפני חודש, גם אחרי שהאדפטרים השתנו. לכן שלושה שדות גרסה,
ולכל אחד תפקיד אחר - אף אחד מהם לא מחליף את האחרים:

| שדה | מקור | למה הוא קיים | השוואה |
|---|---|---|---|
| `injectorVersion` | `INJECTOR_VERSION` ב-`injector.cjs` | semver מסודר וקריא לאדם. **השדה היחיד שהשער משווה** | `<` מותר (סדר אמיתי) |
| `displayName` | `BOOKMARK_DISPLAY_NAME` ב-`injector.cjs` | **שם הסימנייה המדויק** שהלקוח גורר, כולל הגרסה | השוואת מחרוזת |
| `version` | hash שהשרת מחשב על ה-build (`server/app.js`) | מזהה build יחיד לתמיכה ולבדיקת ההתקנה | `===` בלבד |

שלושתם חוזרים מ-`GET /api/bookmarklet`: `{ code, bytes, version, injectorVersion, displayName }`. השדות
`injectorVersion` ו-`displayName` **נוספו** ולא החליפו דבר - `version` נשאר כפי שהיה (docs/PIPELINE-CONTRACT:
החוזה המפורסם הוא תוסף בלבד).

**למה השער על ה-semver ולא על ה-hash.** ה-hash מערבב בתוכו את ה-origin, ולכן אותו קוד בדיוק מקבל hash
שונה ב-localhost ובפרודקשן; רשימת hash-ים בצד השרת הייתה נשברת בין הסביבות. בנוסף hash הוא חסר סדר -
`<` עליו הוא באג, לא אופטימיזציה. ה-semver מסודר, זהה בכל סביבה, וגם זה שהלקוח רואה בשורת הסימניות.

**למה `displayName` הוא שדה שרת ולא מחרוזת באתר.** האתר מפנה את הלקוח לסימנייה בשמה ("לחצו על טען
עגלה"). אם הכותרת של הסימנייה שנגררה שונה מהשם שבהוראה, ההוראה מצביעה על שם שלא קיים בשורת הסימניות של
הלקוח. לכן גם הכותרת וגם ההוראות נגזרות מאותו קבוע. החלטת נאור (22.9): השם נשאר `טען עגלה` והגרסה
מתווספת בסוף - `טען עגלה v1.1.0`. אין שינוי שם.

### שער הגרסה

ב-payload, שני שדות **אופציונליים** שה-backend שולח:

| שדה | טיפוס | משמעות |
|---|---|---|
| `minInjectorVersion` | semver | כל injector מתחת לזה מסורב. השער הרגיל: ערך אחד, מונוטוני |
| `blockedInjectorVersions` | מערך semver | משיכה של build בודד תקול, בלי להרים את הרצפה מעל builds תקינים |

**השער רדום כל עוד ה-payload לא מבקש אותו.** אין שדות - אין שער, וה-injector רץ כרגיל. לכן אפשר לפרסם
את ה-injector הזה לפני שה-backend שולח את השדה, ו-payload מ-backend ישן ממשיך לעבוד. `minInjectorVersion`
שלא ניתן לפענח מתעלמים ממנו (טעות backend אינה סיבה לסרב לכל הלקוחות); `blockedInjectorVersions` עדיין פועל.

השער רץ ב-`execute()`, **לפני בקשת ההוספה הראשונה**, ולא ב-`bootstrap` בלבד - כל נקודות הכניסה (סימנייה,
תוסף, WebView) עוברות דרכו, ועגלה שהתמלאה חצי היא בדיוק מה שהוא בא למנוע. כשהוא מסרב: הודעה לעברית
(`alert` + באנר) "הסימנייה שלכם ישנה. חזרו לסל חכם, מחקו אותה וגררו אותה מחדש.", אפס בקשות הוספה, אין
הפניה ל-checkout - ובכל זאת **נשלח דוח**, כדי שהפלטפורמה תדע איזה build סורב:
...
**נוהל שחרור.** כל שינוי התנהגותי ב-`injector.cjs` מעלה את `INJECTOR_VERSION`. ה-hash מתעדכן לבד.
**גם מיזוג הוא שינוי כזה**, גם כשאיש לא נגע בקבוע.

*למה זה כתוב במפורש (22.9).* ה-branch של השער נחתך לפני ש-`cbf2910` לימד את ה-injector לשלוח ק"ג, ולכן
ה-build ממנו ידע לסרב לסימנייה ישנה אבל לא ידע לשלוח משקל. המיזוג `b74f543` הוסיף 107 שורות של שליחת
משקל ל-injector שעדיין קרא לעצמו `1.0.0` - שתי סימניות, אותו semver, התנהגות שונה. זה בדיוק מה שהשער
לא יכול להבחין בו, כי הוא משווה semver. הגרסה הועלתה ל-`1.1.0` וזו הסיבה. (לא היו משתמשים אמיתיים
בחלון הזה - דגלי סטייג'ינג.)

*מה שומר על זה עכשיו.* `weightedContract` ב-`test/injector.test.js` קורא את האדפטרים ושואל את ה-injector
אם הוא יודע לשרת כל אחד מהם שמכריז על נתיב שקיל. ה-branch של השער היה נכשל בו: הטסטים שלו היו ירוקים רק
משום שלא הריצו אף נתיב שקיל. זה לא תופס "שינוי התנהגותי" באופן כללי - שום טסט לא יתפוס - אבל הוא תופס את
הסוג שקרה כאן: injector ואדפטרים שנפרדו זה מזה.
