...
לתשומת לב מרלוג: בדיקת הריצה הזו (`no new concept takes a product whose name says its word is a flavour`) קוראת את `data/products.json` שמרלוג מפרסמת, ולכן היא מפגרת בריצה אחת אחרי שינוי מושגים. בדיקה אדומה עוצרת את הפרסום, אך מאז 22.9 לא את איסוף מחירי הסניפים.
