...
כללים:
- **כלל המקור (18.9.2026, סופי):** המחירון והקטלוג נבנים **אך ורק** מהקבצים שהרשת מפרסמת לפי תקנות שקיפות המחירים, ו**בריצה היומית אין שום פנייה לאתרי הרשתות**. מוצר שאין לו מחיר מפורסם לא מוצג; חנות מקוונת בלי קובץ (יוחננוף מחירון C) לא נבנית עד שהרשת תפרסם. ההשוואות מול האתרים שנעשו ב-17-18.9 (`audit:online-check`, `audit:shufersal-codes`) הן כלי ביקורת ידני להפעלה לפי בקשה, ותוצאותיהן לא נכנסות לקטלוג. מלאי אונליין ותמונות יגיעו ממקור אחר (קובצי הרשתות כשיכללו אותם, או ספק תמונות). ממומש ב-`scripts/build-products.mjs`.
...
(A, B, C), והלקוח בוחר נקודת איסוף. **הוחלט (17.9):** יוחננוף מפוצלת לתתי-רשת פיקאפ, אחת לכל מחירון מפורסם:
`yochananof` = "יוחננוף פיקאפ" (A, קובץ 050, אימות ב-view s116) ו-`yochananof_b` = "יוחננוף פיקאפ בת ים / נס ציונה" (B, קובץ 015,
אימות ב-view s84), שתיהן מסומנות "איסוף עצמי בלבד" עם רשימת נקודות האיסוף. C לא נבנית: אין מחירון מפורסם, ואין מחיר בלי מחירון. פנייה לרשת.
ה-handoff חייב לקבוע את נקודת האיסוף שנבחרה (view) בעגלה, אחרת המחיר בקופה הוא של C.
