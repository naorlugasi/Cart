...
כללים:
- **כלל המקור (17.9.2026, סופי):** המחיר המוצג הוא **אך ורק** מהמחירון שהרשת מפרסמת לפי תקנות שקיפות המחירים. ה-API של אתר הרשת הוא בדיקה פנימית בלבד: **מאמת** (אחוז אי-התאמה ב-`source.online.verify`), מסמן מה לא נמכר אונליין (`in_stock`) ומוסיף תמונות. הוא לעולם לא מקור מחיר, ומוצר שאין לו מחיר מפורסם לא מוצג. חנות מקוונת בלי קובץ מפורסם (יוחננוף מחירון C) לא נבנית עד שהרשת תפרסם. ממומש ב-`scripts/build-products.mjs`.
