# תוכנית מימוש: שירות המחירונים היומי (15.9.2026)
...
| רמי לוי | Cerberus | 039 "מרלוג אינטרנט" | `POST /api/catalog` סניף 331: מחיר, מלאי, תמונה |
| יוחננוף | Cerberus | אין קובץ ייעודי (001 סניף דגל) | GraphQL `products(sku in …)`: מחיר, מלאי, תמונה |
| טיב טעם | Cerberus | 502 ליקוט נתניה | לא צריך |
| קשת טעמים | Cerberus | 120 ממ"ר (לא תמיד מפרסם) | לבדוק Self Point `/products` |
...
כללים:
- שורת המחיר לאונליין = `prices_current` של סניף האונליין, ואם קיים `online_overlay` לאותו (סניף, ברקוד) הוא **גובר** (מחיר ומלאי מהאתר עצמו). ברקוד שקיים בקובץ ולא ב-overlay של רשת שיש לה overlay מלא (חצי חינם) מסומן `in_stock = false`. זה בדיוק הכלל שקיים היום ב-`build-products.mjs`.
