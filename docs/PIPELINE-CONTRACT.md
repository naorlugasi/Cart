...
### 2.1 `data/products.json` - כל מוצר שרשת כלשהי מוכרת (מ-23.9: ~48,000; קודם ~9,000 בסף 3 רשתות)

**אין מוצר מוחרג (החלטת נאור, 23.9).** עד 23.9 מוצר נכנס רק אם 3 רשתות מכרו אותו (או שהיה מותג פרטי), ותקרה הגבילה את המשותפים ל-6,000. הכלל הוסר: רשת שנותנת למוצר ברקוד משלה ושם משלה עושה את זה כדי שאי אפשר יהיה להשוות את מחירו, והפיכתו להשוואתי היא העבודה. `--min-chains` ו-`--max` עדיין קיימים לבנייה מקומית קטנה. מה שהגודל יוצר נענה בפיצול למחלקות (§2.1.1), לא בהסתרת מוצרים: הקובץ המלא ~21MB (1.65MB דחוס), ומי שיקרא שבר מחלקה יוריד את הכבדה ביותר ב-4.4MB שהם 346KB על החוט. **נכון ל-23.9 אף צרכן לא קורא את השברים**, ולכן הפיצול הוא אפשרות פתוחה ולא חיסכון בפועל; הגודל של היום נסבל אצל הצרכן היחיד שיש (מדידת הבקאנד: 93ms פרסור, 1.2s אינדוקס, תשובות דחוסות). **`basePrice` של מוצר שרשת אחת מוכרת הוא המחיר של אותה רשת, לא מחיר שוק** - אסור להציג אותו כממוצע.
...
- `conceptFamily` (`{ id, name } | null`, 23.9): תת-הקטגוריה של המושג בתוך המחלקה שלו ("פטריות" → שמפיניון/פורטובלו, "תפוחים" → זהוב/גרנד סמית/חרמון/פינק ליידי) - כך שהפרונט/בקאנד יכולים להציג צ'יפים של סינון בתוך מחלקה בלי לגזור את הקיבוץ בעצמם (`familiesForCategory`, `src/catalog/concepts.js`). `null` רק כשלמוצר אין `conceptId` בכלל; לכל מושג יש תמיד משפחה - מושג בלי אחים באותה מחלקה הוא המשפחה של עצמו. שדה תוספתי, מחושב מה-`conceptId` הסופי (אחרי השומר ב-`conceptForCategory`), ולכן מוצר שאיבד את המושג שם מאבד גם את המשפחה ולא נשאר עם משפחה שגויה. תיעוד מלא: docs/CONCEPTS.md §10.
  **מובטח: `conceptId` קיים ⇔ `conceptFamily` קיים.** לכל מושג יש משפחה (מושג בודד הוא המשפחה של
  עצמו), ושניהם נגזרים מאותו `conceptId` סופי, ולכן ספירת המוצרים עם מושג וספירת המוצרים עם משפחה
  זהות תמיד (24.9: 21,839 כל אחת). זו הבטחה מכוונת ולא צירוף מקרים. **ועדיין: צרכן שסופר את
  השניים צריך לספור כל אחד בנפרד ולא לגזור אחד מהשני** - הבטחה היא הכי מסוכנת ברגע שצרכן בונה
  עליה, ואם היא תרופף אי-פעם, הפער צריך להופיע כהבדל בין שני מספרים ולא לשבור חישוב.
...
### 2.1.1 `data/products/<slug>.json` ו-`data/products-index.json` - שברי מחלקה (מ-23.9)

> **מצב היום (23.9): הקבצים מתפרסמים ואיש עדיין לא קורא אותם.** `products.json` הוא הקובץ האוטוריטטיבי, וה-cartBackend מושך אותו ואת `catalogs/<chain>.json` בלבד. כלומר הפיצול **זמין ולא נדרש**: הוא לא מקטין היום שום משיכה בפועל, והוא יהפוך למועיל ביום שצרכן יבחר לקרוא אותו (הבקאנד תחת לחץ זיכרון, או צרכן אחר). **אל תמחקו את `products.json` מהפרסום** כי נראה ששברי המחלקה "מחליפים" אותו - הם לא, עד שיוחלט אחרת בשינוי גרסה מוסכם. קובץ שאיש לא קורא הוא בדיוק מה שנמחק בשקט בעוד חצי שנה; זו הסיבה שהמשפט הזה כתוב כאן.

**הרקע**: `products.json` עמד לגדול מ-9,000 מוצרים (3.7MB) לכ-48,000 (19MB) ברגע שסף 3 הרשתות והתקרה של 6,000 יוסרו (שינוי עתידי, לא בתאריך הזה); צרכן קר שמושך את כל הקטלוג היה מוריד עשרות MB בבת אחת. הפתרון: לפצל את `products.json` לפי מחלקה, כך שצרכן מוריד מחלקה אחת ולא את כל הקטלוג. **השינוי הזה תוספתי בלבד - הוא לא נוגע בסף 3 הרשתות ולא בתקרת 6,000; `products.json` ממשיך להיכתב בדיוק כמו היום ונשאר המשטח הרשמי של החוזה עד שינוי גרסה מפורש שיסיר אותו (§3).**

- `data/products/<slug>.json`: מערך באותו פורמט בדיוק כמו איברי `products.json` (§2.1) - **בדיוק** המוצרים שה-`category` שלהם הוא המחלקה הזו. `<slug>` הוא מזהה ascii יציב למחלקה, לא תעתיק על הטייס - הוא נשמר ב-`DEPARTMENT_SLUGS` ליד רשימת 13 המחלקות (`src/catalog/categorize.js`, מיוצא) ולא מחושב מחדש בשום מקום אחר:
  ...
  מוצר שה-`category` שלו חסר או אינו אחת מ-13 אלה (תקלת נתונים - `categorize()` עצמה תמיד מחזירה אחת מהן, עם `כללי` כברירת מחדל) נופל לשבר `other.json` ("אחר") במקום להיזרק - כך שכל מוצר מופיע בשבר אחד ובדיוק אחד, בלי אף מוצר שנעלם.

- `data/products-index.json`: `{ "version": 1, "buildId": "<ISO של רגע הבנייה>", "generatedAt": "<אותו ISO>", "departments": [ { "id": "<slug>", "name": "<שם המחלקה בעברית>", "file": "products/<slug>.json", "count": <מספר מוצרים>, "bytes": <גודל הקובץ> } ], "total": <סה"כ מוצרים> }`. **זה מה שהשרת קורא כדי לגלות את השברים** - בלי `readdir` (אין כזה מעל HTTP), באותו רעיון בדיוק כמו `config/concepts/index.json` (`src/catalog/concepts.js`, `INDEX_FILE`). **צרכן קורא את הרשימה מהאינדקס ולעולם לא מקודד רשימת מחלקות קשיחה בקוד שלו** - הוספת מחלקה 14 צריכה לדרוש עריכה אחת בלבד (`DEPARTMENT_SLUGS`) ולא שינוי קוד אצל שום צרכן. שבר של מחלקה שהתרוקנה (0 מוצרים) לא נכתב ולא מופיע באינדקס; קובץ ישן שלה על הדיסק נמחק בבנייה הבאה, כדי שהאינדקס תמיד יתאר בדיוק את מה שקיים.

- `scripts/build-products.mjs` מדפיס אזהרה נפרדת אם שבר בודד עובר 4MB (הגודל שצרכן בפועל מוריד), בנוסף לאזהרת הגודל הקיימת על `products.json` כולו; טבלת השברים (מחלקה, ספירה, MB) מודפסת בסיכום הבנייה, ממוין לפי גודל.
...
- **שברי המחלקה (§2.1.1) הם תוספתיים**: `data/products/<slug>.json` ו-`data/products-index.json` הם קבצים חדשים לגמרי, ו-`products.json` עצמו לא השתנה - שום שדה בו לא נמחק ולא שינה משמעות. צרכן שלא מכיר את הקבצים החדשים ממשיך לעבוד בדיוק כמו היום; רק צרכן שבוחר לעבור לקריאה לפי מחלקה קורא אותם.
