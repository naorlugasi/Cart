...
- **הגדרות המושגים ל-cartBackend (20.9):** ה-API החדש טוען את `config/concepts/*.json` יחד עם `data/` (שמות נרדפים לחיפוש, `sizeUnit` לבדיקת גודל בתחליפים, שם המושג לקיבוץ תוצאות). מדיסק זה readdir; מ-HTTP (GitHub raw / R2) אי אפשר לקבל רשימת קבצים, ולכן השרת מנסה `config/concepts/index.json` ואם אין - רשימת שמות מוטמעת (9 המחלקות של היום). **בקשה: לפרסם `config/concepts/index.json` עם מערך שמות הקבצים** (או קובץ מאוחד `concepts.json` ליד `products.json`), כדי שהוספת מחלקה חדשה לא תדרוש שינוי קוד בשרת. עד אז, מחלקה חדשה שנוספת לא תיטען בייצור.
