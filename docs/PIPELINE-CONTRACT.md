...
> **שדה חדש לא מגיע לבד.** `cartBackend` בונה כל מוצר שדה-שדה מהרשומה של הצינור (לא spread), ולכן שדה שנוסף כאן נשמט בשקט
> עד שהבקאנד קורא לו בשמו - ככה `verified` לא הגיע לפרונט במשך שעות ב-23.9. מי שמוסיף שדה כאן מודיע לסשן הבקאנד ולסשן הפרונט
> במפורש: שם השדה, הטיפוס, אילו מוצרים נושאים אותו ומתי הוא חסר.
...
- `conceptFamily` (`{ id, name } | null`, 23.9): תת-הקטגוריה של המושג בתוך המחלקה שלו ("פטריות" → שמפיניון/פורטובלו, "תפוחים" → זהוב/גרנד סמית/חרמון/פינק ליידי) - כך שהפרונט/בקאנד יכולים להציג צ'יפים של סינון בתוך מחלקה בלי לגזור את הקיבוץ בעצמם (`familiesForCategory`, `src/catalog/concepts.js`). `null` רק כשלמוצר אין `conceptId` בכלל; לכל מושג יש תמיד משפחה - מושג בלי אחים באותה מחלקה הוא המשפחה של עצמו. שדה תוספתי, מחושב מה-`conceptId` הסופי (אחרי השומר ב-`conceptForCategory`), ולכן מוצר שאיבד את המושג שם מאבד גם את המשפחה ולא נשאר עם משפחה שגויה. תיעוד מלא: docs/CONCEPTS.md §10.
  **המשפחות של מחלקה לעולם אינן מחלקות אותה:** מוצר בלי `conceptId` אין לו משפחה, והוא נגיש רק דרך הרשימה הלא-מסוננת. לכן סכום המשפחות של מחלקה תמיד קטן מהמחלקה עצמה, וההפרש הוא בדיוק המוצרים בלי מושג (כשליש מהקטלוג). צרכן שמקבץ לפי משפחה חייב לפרסם גם ספירה של הלא-משויכים (`unclassified`) בנפרד מרשימת המשפחות, ולא כמשפחה מדומה - צ'יפ כזה היה גדול מכל משפחה אמיתית ומוביל לרשימה חסרת קשר.
...
- `name` = השם הנפוץ ביותר בין הרשתות, ובעדיפות לשם לא-קצוץ (כמחצית הרשתות קוצצות שמות ל-~20 תווים; `size` ו-`conceptId` נגזרים מכל השמות של הברקוד, לא רק מהמוצג). מ-22.9 שם תצוגה ידני מ-`config/categories/names.json` גובר כשקיים (מוצרים בודדים שהשם הנפוץ שלהם סתום), והשם הנפוץ עובר אז ל-`aliases` - לכן `aliases` כבר אינו תמיד ריק, וחיפוש צריך לכלול אותו. `brand` יכול להיות `","` או ריק (איכות נתונים ידועה). `category` מ-13 קטגוריות (עשר מ-20.9, ומ-23.9 גם `תינוקות`, `בעלי חיים` ו-`בית וכלים`; הוספת מפתח היא תוספתית - צרכן שלא מכיר מפתח מציג אותו בשמו) - **הקבצים לא מכילים קטגוריה**; היא נקבעת אצלנו. מ-20.9 כל מוצר בקטלוג עבר סקירה פרטנית מול טקסונומיה כתובה (`docs/CATEGORIES.md`) והתוצאה שמורה ב-`config/categories/labels.json`; מילות המפתח ב-`src/catalog/categorize.js` הן רק נפילה אחורה למוצר חדש שטרם נסקר. "כללי" ירד מ-1,051 ל-187 מוצרים. `icon` הוא שריד להצגה; לא לבנות עליו. `basePrice` הוא **חציון המחיר בין הרשתות שמוכרות את המוצר** - כשרשת אחת מוכרת אותו זהו פשוט המחיר שלה, לא מחיר שוק, ואין להציג אותו כ"המחיר הממוצע" (23.9). מ-23.9 **מוצר מושג שקיל** (`kind: 'concept'`) נושא גם `sources: [{chain, name, price}]` - השורה השקילה שכל רשת תרמה לחציון, מהזולה ליקרה **על פני כל משפחות הרשתות שבנתונים ולא רק אלה שהאתר מציג** - `sources[0]` הוא הזול בארץ ולא הזול אצל הרשת שהמסך מנקב בשמה, ולכן צרכן שכותב "הזול: יוחננוף" חייב לחפש ברשימה את הרשומה עם ה-`chain` הזה במקום לקחת את הראשונה (נתפס בפרונט, 23.9). הכרטיס הוא חציון על שורה אחרת בכל רשת, ולכן הוא חייב להגיד איזו: "פטריות" הוא שמפיניון ברשת אחת ופורטובלו באחרת (החלטת נאור 23.9). שדה תוספתי, קיים רק על מוצרי מושג; הפרונט מוזמן להציג "ברמי לוי: תפוח עץ גרנד סמיט ₪7.90". **`isWeighted` הוא המקור היחיד ליחידת המחיר, ואסור לגזור אותו**: לא מ-`gtin` חסר, לא מ-`kind`, לא מ-`unit`. בקטלוג המפורסם היום 281 מוצרים שקילים, מהם **241 עם ברקוד** (בשר טחון טרי, המבורגר, פילה סלמון, אבוקדו ארוז) ורק 40 כרטיסי מושג; מי שגוזר "שקיל = בלי ברקוד" יציג ל-241 מוצרים מחיר לקילו בלי לומר שהוא לקילו - ₪99.90 להמבורגר נקרא כמחיר האריזה. **`isWeighted` אינו מזהה של כרטיס מושג**: הוא אומר שהמחיר הוא לק"ג, ובקטגוריית ירקות ופירות 49 מ-82 המוצרים השקילים הם מוצרים עם ברקוד ("אבוקדו ארוז ברשת", "עגבניה מגי") שלעולם לא יישאו `sources` (נמדד בפרונט, 23.9). מי שצריך להבחין בין כרטיס מושג למוצר שקיל עם ברקוד יבדוק `kind === 'concept'`, או פשוט את נוכחות `sources`. דוגמה אמיתית מהבנייה: [docs/samples/weighed-concept-products.json](samples/weighed-concept-products.json).

 מ-23.9 כל מוצר נושא גם `verified: boolean` (תוספתי): `true` כשיש לו רשומה ב-`config/products/verified.json` (docs/PLAN-PRODUCT-TRUTH.md §2) - שם/מחלקה/מושג/גודל שאדם, סבב סקירה או שלוש חנויות מקוונות אישרו. הפרונט מציג לא-מאומת רגיל (החלטת נאור 23.9).
...
- הקטלוג הדק **לא נושא** `conceptFamily` (23.9) - השדה קיים רק על מוצר ב-`products.json`. פריט שקיל בקטלוג הדק ממשיך לשאת `conceptId` בלבד (§2.1 §6), וצרכן שצריך את המשפחה מחפש את המוצר המתאים ב-`products.json` או קורא ישירות מ-`familiesForCategory`.
...
- **`out_of_stock` בשורת השוואה אינו "אזל מהמלאי"** (23.9): אחרי תיקון `ItemStatus` המקור היחיד שנותר ל-`inStock: false` הוא
  `applySiteCodes` - ברקוד שאתר שופרסל עצמו לא מכיר, כלומר **לא נמכר באונליין** ברשת הזו, קבוע ולא זמני. 12 מוצרים היום.
  התווית שהלקוח רואה צריכה להיות "לא נמכר באתר הרשת" ולא "אזל"; שם הסטטוס נשאר כפי שהוא כדי לא לשבור את שני הצרכנים
  בשביל 12 מוצרים.
- `inStock` הוא **תמיד `true`** היום (אין מקור חוקי למלאי אונליין). לא לבנות עליו. כך היה כתוב כאן מההתחלה, אבל הקוד קרא
  `ItemStatus` מקובצי השקיפות כאילו הוא מלאי, ורק 3 מ-14 הרשתות מפרסמות את השדה בכלל - שוק העיר מפרסמת "0" בכל 3,166
  השורות שלה. התוצאה בייצור: **כל שורה של שוק העיר בכל השוואה הוצגה "אזל מהמלאי" והסל שלה היה ₪0**, רשת שלמה שלא יכלה
  לנצח. תוקן 23.9 (`src/catalog/priceXml.js`) אחרי שנאור דיווח שמוצר שהאתר הציג כזמין אזל ברשת. השדה חוזר ל-`true` תמיד
  מקובץ מחירים; זמינות אמיתית מגיעה רק משכבת החנות המקוונת (`online.json`, 4 רשתות), ושם **מוצר שאינו מופיע ברשימה** הוא
  הסימן - החנויות לא מדווחות "אזל", הן פשוט לא מציגות. לכן היום אין לנו סימן מלאי אמין לאף רשת, והמקום היחיד שבו הוא
  קיים הוא רגע ההעברה לעגלה, שבו ההזרקה מחפשת את הפריט באתר החי.
