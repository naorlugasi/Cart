...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .btn, .card, .chip-btn, .row button, .tab { touch-action: manipulation; user-select: none; -webkit-user-select: none; }
  ...
  .shelf { display: grid; grid-auto-flow: column; grid-auto-columns: minmax(250px, 1fr); gap: 14px; overflow-x: auto; padding: 4px 2px 12px; scroll-snap-type: x mandatory; scrollbar-width: none; margin-inline: -2px; }
  .shelf::-webkit-scrollbar { display: none; }
  .card { scroll-snap-align: start; background: var(--card); border-radius: 12px; overflow: hidden; border: 2px solid transparent; box-shadow: var(--shadow); cursor: pointer; text-align: right; padding: 0; display: grid; grid-template-rows: auto 1fr; transition: transform 200ms var(--ease-out), border-color 150ms ease, box-shadow 200ms var(--ease-out); position: relative; }
  .card.on { border-color: var(--blue); }
  .card:active { transform: scale(.985); }
  .cover { position: relative; aspect-ratio: 16 / 9; background: var(--surface); display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; }
  .cover img { width: 100%; height: 100%; object-fit: cover; }
  .cover .logo { position: absolute; inset-inline-start: 10px; bottom: -14px; width: 40px; height: 40px; border-radius: 10px; color: #fff; display: grid; place-items: center; font-family: "Fredoka", sans-serif; font-weight: 700; font-size: 18px; box-shadow: 0 2px 6px rgba(0,0,0,.2); border: 2px solid var(--card); }
  .cover .badge { position: absolute; top: 10px; inset-inline-start: 10px; background: var(--blue); color: #fff; font-weight: 600; font-size: 12px; padding: 4px 10px; border-radius: 999px; }
  .cover .badge.warn { background: var(--card); color: var(--warn); }
  .cbody { padding: 22px 14px 14px; display: grid; gap: 6px; }
  .cbody .name { font-weight: 600; font-size: 16px; display: flex; justify-content: space-between; align-items: baseline; gap: 10px; }
  .cbody .name .num { font-weight: 600; font-size: 18px; }
  .cbody .name .num.partial { color: var(--ink-2); }
  ...
  .card.blocked .cover { filter: grayscale(.6); }
  ...
  .imgs { display: none; }
  @media (hover: hover) and (pointer: fine) { .card:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0,0,0,.08), 0 18px 40px rgba(0,0,0,.12); } .btn:not([disabled]):hover { filter: brightness(1.06); } }
  @media (prefers-reduced-motion: reduce) { .card, .btn { transition: none; } .card:hover, .card:active, .btn:active { transform: none; } .btn .l .lbl.swapping { filter: none; } }
  @media (max-width: 760px) { .top-in { gap: 8px; } .addr .full { display: none; } .addr .shortaddr { display: inline; } .basket-btn { padding: 8px 12px; } .info { grid-template-columns: 1fr 1fr; } .shelf { grid-auto-columns: 78%; } h1 { font-size: 26px; } .top-in { padding-inline: 16px; } .wrap, .menu { padding-inline: 16px; } .brandmark { font-size: 20px; } }
</style>
...
(function () {
  ...
  function render() {
    ...
    $('shelf').innerHTML = R.map(function (r, i) {
      ...
      return '<button type="button" class="card' + (r.c === sel ? ' on' : '') + (r.belowMin ? ' blocked' : '') + '" data-c="' + r.c + '" aria-pressed="' + (r.c === sel ? 'true' : 'false') + '"><div class="cover">' + pics + badge + '<span class="logo" style="background:' + D.brand[r.c] + '" aria-hidden="true">' + D.initials[r.c] + '</span></div><div class="cbody"><div class="name"><span>' + esc(c.name) + '</span><span class="num' + (r.complete ? '' : ' partial') + '">' + nis(r.total) + '</span></div><div class="meta"><span>' + I.truck + (r.fee ? 'משלוח ' + nis(r.fee) : 'משלוח חינם') + '</span><span>' + I.clock + esc(c.eta) + '</span></div><div class="meta">' + status + (r.sw || r.cheaper ? '<span>' + (r.sw + r.cheaper) + ' החלפות</span>' : '') + '</div></div></button>';
    }).join('');
  ...
  function bind(cur, mn) {
    ...
    on('.card', function (el) { sel = el.getAttribute('data-c'); render(); document.querySelector('.card[data-c="' + sel + '"]').focus({ preventScroll: true }); });
