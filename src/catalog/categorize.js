...
export const CATEGORY_RULES = [
  ...
  wordRule('תינוקות',
    `לתינוק|תינוקות|תינוק${NOT_HEB_AHEAD}|לפעוט|פעוטות|מטרנה|סימילאק|נוטרילון|תמ"ל|תרכובת מזון|גרבר|חטיפטף|פרינוק|פריפלצת|האגיס|פמפרס|בייביסיטר|במבינו|טיטולים|חיתול|החתלה|מוצץ|נשכנ|כוס הפלא|קערת האכלה|ג'ונסונס|קמיל בלו|טלק${NOT_HEB_AHEAD}|ניו ?בורן|בקבוק לתינוק|בקבוק לגדולים|כפי(?:ות|ת) ?(?:סיליקונ?|האכלה)|בייבי(?! בל)`,
  ...
  wordRule('בית וכלים',
    ...
    (name) => (/מגש/.test(name) && /עגלה|קצבים|צלי |לקט|עלים(?!ה)|טארט|פלמני/.test(name))
      ...
      // "כפית" (25.9, added beside the plural the rule already had) has one real false positive and it is
      // a whole product line: a tabletop sweetener is sold as "כפית לכפית" ("spoon for spoon", the sugar
      // equivalence), and the chains truncate that to "...כפית ל" mid-phrase, so a lookahead on the full
      // phrase misses five of them. The brand and the category word are what identify it, not the spoon.
      || (/כפית/.test(name) && /סוכרזית|ממתיק|סטיוויה|סוכרלוז/.test(name))),
...
const PROCESSED_MINUS_MUSHROOM_FORM = /מוחמצ|במי מלח|מתבל|חומץ|משומר|מיץ|נקטר(?!ינ)|בטעם|טעם |סירופ|מחית|קפוא|מוקפא|כבוש|בסירופ|ריב[הת]|חטיפ|טוגנ|מצופ|גומי|מ"ל|ליטר|בקבוק|פחית|קופס|קלוי|מטוגן|רצועות|שלישיית|רביעיית|רכז|תרכיז|צנצנת|שפופרת|במילוי|קצוצ|ממתק|כיסונ|קוביות|ממרח|רוטב|פרוט ?(&|אנד) ?ווג|גלידה|סרבט|פסטה|פסטו|תיבולית|עוג[הת]|מאפין|מרק|נמס בכוס|מנה חמה|שימור|לפתן|פריפלצת|מיונז|סלט|ברוסקט|חטיף|קאיין|טחון|מעדן|לחם|בריזר|צ'?יפס|שמן|איולי|משקה|\d\s*%|אסקימו|מרציפן|נטורטינט|פרוטיבר|חמצוצ|תמצית|כמוסות/;
